// app/services/interior/_components/InteriorComparison.tsx

import { Drill, Plug, CheckCircle, HelpCircle } from "lucide-react";
...
export default function InteriorComparison() {
  return (
      ...
      <div className="container mx-auto px-4 max-w-[1400px]">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 mb-4 tracking-tight">
            {interiorDetails.comparisonTitle}
          </h2>
          <p className="text-slate-500 text-lg max-w-2xl mx-auto leading-relaxed">
            {interiorDetails.comparisonDesc}
          </p>
        </div>

        <div className="grid md:grid-cols-2 gap-8 items-stretch">
          {/* Сторона А (Чистовая установка) */}
          <div className="bg-white p-8 md:p-10 rounded-3xl border border-orange-100 relative overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
            {interiorDetails.comparisonA.badge && (
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="p-3.5 bg-orange-50 rounded-2xl text-orange-600 border border-orange-100">
                {renderIcon(interiorDetails.comparisonA.iconName || "Drill", "w-6 h-6 text-orange-600")}
              </div>
              <h3 className="text-2xl font-bold text-slate-900">{interiorDetails.comparisonA.title}</h3>
            </div>
            <ul className="space-y-4 text-slate-600 text-sm md:text-base">
              {interiorDetails.comparisonA.items.map((item, idx) => (
          ...
          {/* Сторона Б (Проектирование кабелей) */}
          <div className="bg-white p-8 md:p-10 rounded-3xl border border-slate-200 relative overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-3.5 bg-orange-50 rounded-2xl text-orange-600 border border-orange-100">
                {renderIcon(interiorDetails.comparisonB.iconName || "Plug", "w-6 h-6 text-orange-600")}
              </div>
              <h3 className="text-2xl font-bold text-slate-900">{interiorDetails.comparisonB.title}</h3>
            </div>
            <ul className="space-y-4 text-slate-600 text-sm md:text-base">
              {interiorDetails.comparisonB.items.map((item, idx) => (
                <li key={idx} className="flex items-start gap-3">
                  <CheckCircle className="w-5 h-5 text-orange-600 shrink-0 mt-0.5" />
                  <span className="leading-relaxed">
                    <strong className="text-slate-800 font-bold">{item.bold}</strong> {item.normal}
