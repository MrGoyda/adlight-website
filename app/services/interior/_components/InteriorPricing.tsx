// app/services/interior/_components/InteriorPricing.tsx

import Link from "next/link";
import { Calculator } from "lucide-react";
import { interiorDetails } from "@/dictionaries/services/details/interior";

export default function InteriorPricing() {
  return (
        ...
        <div className="bg-gradient-to-r from-orange-55/80 to-orange-100/55 border border-orange-100 rounded-3xl p-8 md:p-12 flex flex-col md:flex-row gap-12 items-center shadow-sm">
          <div className="md:w-1/2 w-full">
            <h2 className="text-3xl font-extrabold text-slate-900 mb-4 tracking-tight">
              {interiorDetails.pricingTitle}
            </h2>
            <p className="text-slate-600 mb-6 leading-relaxed">
              {interiorDetails.pricingDesc}
            </p>
            ...
            <div className="overflow-hidden rounded-2xl border border-orange-200/60 bg-white shadow-sm">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
                  {interiorDetails.pricingItems.map((pr, i) => (
          ...
          <div className="md:w-1/2 text-center w-full">
            ...
            <p className="text-slate-500 text-sm mb-6 leading-relaxed">
              Например: {interiorDetails.pricingCalculatorPlaceholder}. Консультация нашего инженера и выезд на замер бесплатно.
            </p>
            <Link 
              href="/calculator" 
            ...
            >
              {interiorDetails.pricingActionText}
            </Link>
          </div>
        </div>
      </div>
