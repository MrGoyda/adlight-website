// app/services/pylons/_components/PylonsHero.tsx
"use client";
...
interface PylonsHeroProps {
  heroImages: string[];
}

export default function PylonsHero({ heroImages }: PylonsHeroProps) {
    ...
    <header id="hero" className="relative pt-32 pb-20 lg:pt-40 lg:pb-32 overflow-hidden border-b border-slate-200/80 bg-slate-50">
      ...
      <div className="container mx-auto px-4 max-w-[1400px] relative z-10">
        ...
        <nav aria-label="Хлебные крошки" className="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-slate-500 mb-8 font-medium">
          ...
          <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-orange-600 font-semibold">{pylonsDetails.badge}</span>
        </nav>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            ...
            <p className="text-slate-600 text-lg mb-8 leading-relaxed max-w-xl">
              {pylonsDetails.subtitle} Изготовление отдельно стоящих конструкций для АЗС, ТРЦ и автосалонов. Расчет фундамента КМ/КЖ.
            </p>
          ...
          <div className="relative aspect-square rounded-3xl bg-slate-100 border border-slate-200/80 overflow-hidden group shadow-xl">
            ...
            <div className="absolute bottom-6 left-6 right-6 bg-white/95 backdrop-blur-md border border-slate-200/80 p-4 rounded-2xl flex items-center gap-4 pointer-events-none z-20 shadow-lg">
              ...
              <div>
                <div className="text-slate-900 font-bold text-base">Капитальные конструкции</div>
                <div className="text-slate-500 text-xs font-medium">Безопасность подтверждена расчетами СНиП РК</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <ConsultationModal 
        ...
        source="Услуга: Стелы и пилоны"
        title="Заказать консультацию инженера по стелам"
        subtitle="Оставьте ваши контакты, и мы перезвоним для обсуждения высоты, размеров фундамента, ветровых нагрузок и бюджета конструкции."
