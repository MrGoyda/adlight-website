// app/services/entrance-groups/_components/EntrancePricing.tsx
"use client";
...
export default function EntrancePricing() {
  return (
        ...
        <div className="bg-gradient-to-r from-orange-50 to-orange-100/50 border border-orange-100 rounded-3xl p-8 md:p-12 flex flex-col md:flex-row gap-12 items-center shadow-sm">
          <div className="md:w-1/2 w-full">
            <h2 className="text-3xl font-extrabold text-slate-900 mb-4 tracking-tight">
              {entrance_groupsDetails.pricingTitle}
            </h2>
            <p className="text-slate-650 text-sm md:text-base mb-6 leading-relaxed">
              {entrance_groupsDetails.pricingDesc}
            </p>
            ...
            <div className="overflow-hidden rounded-2xl border border-orange-200/60 bg-white shadow-sm">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
                  {entrance_groupsDetails.pricingItems.map((pr, i) => (
          ...
          <div className="md:w-1/2 text-center w-full">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2">Хотите индивидуальный дизайн-проект?</h3>
            <p className="text-slate-500 text-sm mb-6 leading-relaxed">
              Объект: {entrance_groupsDetails.pricingCalculatorPlaceholder}. Сделаем замер фасада и подготовим фотопривязку бесплатно.
            </p>
            <Link 
              href="/calculator" 
              className="inline-flex items-center justify-center px-8 py-4 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-2xl transition shadow-sm active:scale-95 text-base"
            >
              {entrance_groupsDetails.pricingActionText}
            </Link>
          </div>
        </div>
      </div>
