  ...
  return (
    <section id="technologies" className="py-24 bg-slate-50 border-y border-slate-200/80 relative w-full overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="flex overflow-x-auto w-full pb-8 gap-4 snap-x snap-mandatory md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-6 md:overflow-visible md:pb-0 hide-scrollbar">
          {VOLUME_LETTERS_CATALOG.map((type) => (
            <Link 
              href={`/services/volume-letters/${type.slug}`} 
              key={type.id}
              className="group relative flex flex-col w-[82vw] max-w-[320px] md:w-auto md:max-w-none md:min-w-0 snap-center rounded-3xl bg-white border border-slate-200 overflow-hidden hover:border-orange-500/50 hover:shadow-xl hover:shadow-orange-500/5 transition-all duration-300 cursor-pointer h-[420px] shrink-0"
            >
              ...
              <div className="p-6 flex flex-col flex-1 justify-between bg-white">
                <div>
                  <h3 className="text-xl font-bold text-slate-900 mb-2 leading-tight group-hover:text-orange-600 transition-colors">
                    {type.title}
                  </h3>
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
                  <span className="text-slate-800 font-bold text-sm bg-slate-50 border border-slate-100 px-3 py-1 rounded-lg">
                    {type.price}
                  </span>
