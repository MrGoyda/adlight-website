// app/services/navigation/_components/NavigationPricing.tsx

import Link from "next/link";
import { Calculator } from "lucide-react";
import { navigationDetails } from "@/dictionaries/services/details/navigation";

export default function NavigationPricing() {
  return (
    <section className="py-24 bg-white border-t border-slate-200/80">
      <div className="container mx-auto px-4 max-w-[1400px]">
        <div className="bg-gradient-to-r from-orange-55/80 to-orange-100/55 border border-orange-100 rounded-3xl p-8 md:p-12 flex flex-col md:flex-row gap-12 items-center shadow-sm">
          <div className="md:w-1/2 w-full">
            <h2 className="text-3xl font-extrabold text-slate-900 mb-4 tracking-tight">
              {navigationDetails.pricingTitle}
            </h2>
            <p className="text-slate-600 mb-6 leading-relaxed">
              {navigationDetails.pricingDesc}
            </p>
            ...
            <div className="overflow-hidden rounded-2xl border border-orange-200/60 bg-white shadow-sm">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
                  {navigationDetails.pricingItems.map((pr, i) => (
          ...
          <div className="md:w-1/2 text-center w-full">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2">Требуется расчет навигации под ключ?</h3>
            <p className="text-slate-500 text-sm mb-6 leading-relaxed">
              Например: {navigationDetails.pricingCalculatorPlaceholder}. Подготовим спецификацию, подберем материалы и рассчитаем смету.
            </p>
            <Link 
              href="/calculator" 
            ...
            >
              {navigationDetails.pricingActionText}
            </Link>
          </div>
        </div>
      </div>
