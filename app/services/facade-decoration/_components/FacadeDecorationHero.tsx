// app/services/facade-decoration/_components/FacadeDecorationHero.tsx
"use client";
...
interface FacadeDecorationHeroProps {
  heroImages: string[];
}

export default function FacadeDecorationHero({ heroImages }: FacadeDecorationHeroProps) {
    ...
    <header id="hero" className="relative pt-32 pb-20 lg:pt-40 lg:pb-32 overflow-hidden border-b border-slate-200/80 bg-slate-50">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <nav aria-label="Хлебные крошки" className="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-slate-500 mb-8 font-medium">
          ...
          <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-orange-600 font-semibold">{facade_decorationDetails.badge}</span>
        </nav>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            <div className="inline-block px-4 py-1.5 mb-6 text-xs font-bold text-orange-600 bg-orange-50 border border-orange-200 rounded-full uppercase tracking-wider">
              {facade_decorationDetails.heroTag}
            </div>
            ...
            <p className="text-slate-600 text-lg mb-8 leading-relaxed max-w-xl">
              {facade_decorationDetails.subtitle} Профессиональный монтаж композитных панелей, облицовка алюкобондом, оформление витрин и входных групп.
            </p>
          ...
          <div className="relative aspect-square rounded-3xl bg-slate-100 border border-slate-200/80 overflow-hidden group shadow-xl">
            ...
            <div className="absolute bottom-6 left-6 right-6 bg-white/95 backdrop-blur-md border border-slate-200/80 p-4 rounded-2xl flex items-center gap-4 pointer-events-none z-20 shadow-lg">
              ...
              <div>
                <div className="text-slate-900 font-bold text-base">Договор и Гарантия</div>
                <div className="text-slate-500 text-xs font-medium">100% соответствие нормативам СНиП и акимата</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <ConsultationModal 
        ...
        source="Услуга: Оформление фасадов"
        title="Заказать расчет облицовки фасада"
        subtitle="Оставьте ваши контактные данные, и наш специалист свяжется с вами для обсуждения размеров фасада и выбора материалов."
