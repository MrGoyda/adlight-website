// app/services/neon/_components/NeonConcept.tsx

import { Camera, Coins, HelpCircle } from "lucide-react";
import Image from "next/image";
import { neonDetails } from "@/dictionaries/services/details/neon";

interface NeonConceptProps {
...
export default function NeonConcept({ fallbackImage }: NeonConceptProps) {
  return (
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center relative z-10">
            <div>
              <h2 className="text-3xl font-extrabold text-slate-900 mb-6 tracking-tight">
                {neonDetails.conceptTitle}
              </h2>
              <p className="text-slate-600 text-lg leading-relaxed mb-8">
                {neonDetails.conceptDesc}
              </p>

              <div className="space-y-6 mb-8">
                {neonDetails.conceptHighlights.map((hl, i) => (
              ...
              {/* Dynamic Tip / Note Box */}
              {neonDetails.conceptQuote && (
                <div className="bg-amber-50 border-l-4 border-orange-500 p-5 rounded-r-2xl text-slate-700 text-sm leading-relaxed shadow-sm">
                  <span className="font-extrabold text-orange-600 block mb-1">💡 Полезный лайфхак:</span>
                  {neonDetails.conceptQuote.replace("Законный лайфхак: ", "")}
                </div>
              )}
            </div>
