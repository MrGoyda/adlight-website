...
export const EstimateCardItem: React.FC<EstimateCardItemProps> = ({
  ...
  /* ── 1. СВЕРНУТЫЙ (КОМПАКТНЫЙ) РЕЖИМ ── */
  if (isCollapsed) {
    const supplierName =
      selectedPrice?.supplier ||
      (item.type === "MATERIAL_SUPPLIER" && item.name.includes(":")
        ? item.name.split(":")[0].replace("[", "").replace("]", "").trim()
        : null);

    const cleanItemName =
      item.type === "MATERIAL_SUPPLIER" && supplierName && item.name.startsWith(`${supplierName}:`)
        ? item.name.slice(supplierName.length + 1).trim()
        : item.name;
    ...
    return (
      <div
        ...
        className="bg-white hover:bg-slate-50/90 p-3 sm:p-3.5 rounded-2xl border border-slate-200/90 shadow-2xs transition-all cursor-pointer space-y-2.5 animate-in fade-in duration-150 group scroll-mt-3"
      >
        {/* Верхний этаж: Ярлык категории + Лейбл поставщика + Кол-во и Сумма */}
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {/* Лейбл поставщика */}
            {supplierName && (
              <span className="px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider bg-purple-100/80 text-purple-800 border border-purple-200/90">
                {supplierName}
              </span>
            )}
            ...
            {item.type === "MATERIAL_STOCK" && (
              <span className="px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider bg-blue-100/80 text-blue-800 border border-blue-200/90">
                Склад
              </span>
            )}
          </div>

          {/* Количество + Итого продажа */}
        ...
        {/* Центральный этаж: Название товара во всю ширину карточки */}
        <div className="w-full">
          <p className="font-extrabold text-slate-900 text-xs sm:text-sm leading-snug break-words">
            {cleanItemName || "Новая позиция"}
          </p>
        </div>

        {/* Нижний этаж: Финансовые данные и Кнопки действий */}
        <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2 text-xs flex-wrap">
          <div className="flex items-center gap-2 text-[11px] font-bold text-slate-500 flex-wrap">
            <span>
              Себес: <b className="text-slate-800 font-extrabold">{itemCostTotal.toLocaleString()} ₸</b>
            </span>
            <span>•</span>
            <span>
              Наценка:{" "}
              <b className={`font-extrabold ${itemProfit >= 0 ? "text-emerald-600" : "text-rose-600"}`}>
          ...
          <div className="flex items-center gap-1.5 shrink-0 ml-auto">
            <button
              ...
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-slate-700 bg-slate-100 hover:bg-orange-50 hover:text-orange-700 hover:border-orange-200 border border-slate-200/80 font-bold text-[11px] transition cursor-pointer active:scale-95"
            ...
            <button
              ...
              className="p-1 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition cursor-pointer active:scale-90"
