    ...
    <header id="hero" className="relative pt-32 pb-20 lg:pt-40 lg:pb-32 overflow-hidden border-b border-slate-200/80 bg-slate-50">
      ...
      <div className="container mx-auto px-4 max-w-[1400px] relative z-10">
        ...
        <nav aria-label="Хлебные крошки" className="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-slate-500 mb-8 font-medium">
          ...
          <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-orange-600 font-semibold">{data.badge}</span>
        </nav>

        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            <div className="inline-block px-4 py-1.5 mb-6 text-xs font-extrabold text-orange-600 bg-orange-50 border border-orange-200 rounded-full uppercase tracking-wider">
              {data.heroTag}
            </div>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 mb-6 leading-tight tracking-tight">
              <span className="block md:inline-block lg:block xl:inline-block mr-3">
                {data.title.split(" (")[0]}
              </span>
              <span className="block md:inline-block lg:block xl:inline-block text-transparent bg-clip-text bg-gradient-to-r from-orange-500 via-orange-600 to-red-600">
                {data.title.includes("(") ? data.title.slice(data.title.indexOf("(")) : "в Астане под ключ"}
              </span>
            </h1>
            <p className="text-slate-655 text-slate-600 text-lg mb-8 leading-relaxed max-w-xl">
              {data.subtitle} Собственное производство, соответствие нормам Дизайн-кода и гарантия качества.
            </p>
          ...
          <div className="relative aspect-square rounded-3xl bg-slate-100 border border-slate-200/80 overflow-hidden group shadow-xl">
            ...
            {(() => {
              const badgeText = data.heroBadgeText || "Инженерный подход";
              const badgeSubtext = data.heroBadgeSubtext || "Бесплатный расчет нагрузок и соответствия Дизайн-коду";
              const iconName = data.heroBadgeIcon || "FileCheck";
              const IconComp = (Icons as any)[iconName] || Icons.FileCheck;
              return (
                <div className="absolute bottom-6 left-6 right-6 bg-white/95 backdrop-blur-md border border-slate-200/80 p-4 rounded-2xl flex items-center gap-4 pointer-events-none z-20 shadow-lg">
                  ...
                  <div>
                    <div className="text-slate-900 font-bold text-base">{badgeText}</div>
                    <div className="text-slate-500 text-xs font-medium">{badgeSubtext}</div>
                  </div>
                </div>
              );
            })()}
