    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[9990] flex justify-end">
          ...
          {/* ── Выдвижная шторка навигации (60/120 FPS Hardware Accelerated) ── */}
          <motion.div
            ...
            className="relative h-dvh w-full sm:w-[460px] bg-white border-l border-slate-200 z-20 shadow-2xl flex flex-col touch-pan-y [touch-action:pan-y] transform-gpu will-change-transform overflow-hidden select-none"
            role="dialog"
            aria-modal="true"
            aria-label="Навигационное меню"
          >
            {/* Левая грань для свайпа закрытия вправо (Swipe to Close Edge Handle) */}
            <div
              onPointerDown={(e) => dragControls.start(e)}
              className="absolute left-0 top-0 bottom-0 w-5 cursor-ew-resize z-50 flex items-center justify-center touch-none group select-none"
              aria-label="Потяните вправо для закрытия"
            >
              <div className="w-1.5 h-16 rounded-full bg-slate-300 group-hover:bg-slate-400 transition-colors pointer-events-none" />
            </div>

            {/* Drag Handle — верхняя пилюля на мобилке */}
            <div
              onPointerDown={(e) => dragControls.start(e)}
              className="sm:hidden flex justify-center pt-2.5 pb-1 shrink-0 cursor-grab active:cursor-grabbing touch-none select-none"
            >
              <div className="w-10 h-1 rounded-full bg-slate-300 pointer-events-none" aria-hidden="true" />
            </div>

            {/* ── Шапка меню ── */}
            <div
              onPointerDown={(e) => dragControls.start(e)}
              className="flex items-center justify-between px-6 py-3.5 border-b border-slate-200 bg-white shrink-0 cursor-grab active:cursor-grabbing select-none touch-none"
            >
              <span className="text-sm font-black text-slate-900 tracking-widest uppercase pointer-events-none">
                Навигация по сайту
              </span>
              <button
                type="button"
                ...
                className="p-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition cursor-pointer active:scale-95 touch-auto"
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain p-6 space-y-7 scrollbar-thin scrollbar-thumb-slate-200 select-text touch-pan-y [touch-action:pan-y]">
