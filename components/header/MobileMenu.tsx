...
// Порог в px — если шторка сдвинута правее этого значения, она закрывается
const SWIPE_CLOSE_THRESHOLD = 100;
// Минимальная скорость свайпа (px/s) для мгновенного закрытия
...
export default function MobileMenu({ isOpen, onClose, onOpenConsultation }: MobileMenuProps) {
  ...
  // MotionValue для смещения шторки по X (0 = открыта, + = сдвиг вправо к закрытию)
  const dragX = useMotionValue(0);

  // Оверлей затемняется пропорционально смещению шторки
  // dragX: 0 → opacity: 1,  dragX: 300 → opacity: 0
  const overlayOpacity = useTransform(dragX, [0, 300], [1, 0]);

  // Масштаб шторки при свайпе — лёгкое сжатие для ощущения нативности
  const drawerScale = useTransform(dragX, [0, 200], [1, 0.97]);
  ...
  // Открытие: сбрасываем позицию перед анимацией входа
  React.useEffect(() => {
    if (isOpen) {
      dragX.set(0);
      triggerHaptic("light");
  ...
  const handleDragEnd = (_: unknown, info: PanInfo) => {
    ...
    // Закрываем если сдвинуто достаточно далеко ИЛИ скорость свайпа высокая
    if (offset > SWIPE_CLOSE_THRESHOLD || velocity > SWIPE_VELOCITY_THRESHOLD) {
  ...
  return (
    <AnimatePresence onExitComplete={() => dragX.set(0)}>
      {isOpen && (
        <>
          {/* ── Затемнённый оверлей с динамической прозрачностью ── */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.div
            ...
            style={{ x: dragX, scale: drawerScale, transformOrigin: "right center" }}
            onDragEnd={handleDragEnd}

            className="fixed top-0 right-0 h-dvh w-full sm:w-[460px] bg-white border-l border-slate-200 z-[100] shadow-2xl flex flex-col touch-pan-y"
            role="dialog"
            aria-modal="true"
            aria-label="Навигационное меню"
          >
            {/* ── Drag Handle — пилюля сверху (только на мобиле) ── */}
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain p-6 space-y-8 scrollbar-thin scrollbar-thumb-slate-200">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <Link
                      key={page.href}
                      href={page.href}
                      className="flex items-center justify-center text-sm font-extrabold text-slate-700 p-3.5 rounded-xl bg-slate-50 border border-slate-200/60 hover:border-orange-500/20 hover:bg-slate-100/50 hover:text-orange-600 transition duration-200 text-center active:scale-95"
                      onClick={() => { triggerHaptic(); onClose(); }}
                    >
                      {page.label}
                    </Link>
                    ...
                    return (
                      <div key={group.id} className="border border-slate-200 rounded-2xl bg-slate-50/50 overflow-hidden">
                        ...
                        >
                          ...
                          <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-300 ${isGroupActive ? "rotate-180 text-orange-600" : ""}`} />
                        </button>
                        <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isGroupActive ? "max-h-[500px] opacity-100 border-t border-slate-200" : "max-h-0 opacity-0"}`}>
              ...
              {/* Группа 3: Технологии объёмных букв */}
            ...
            <div className="p-6 pb-safe-6 bg-slate-50 border-t border-slate-200 space-y-4 shrink-0">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Button
                  variant="solid"
                  onClick={() => { triggerHaptic("medium"); onClose(); onOpenConsultation(); }}
